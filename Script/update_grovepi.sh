...
install_rfrtools_repo() {
  ...
  if [[ $install_rfrtools = "true" ]]; then
    curl --silent -kL https://raw.githubusercontent.com/chaosAD/RFR_Tools/$selectedbranch/scripts/install_tools.sh > $PIHOME/.tmp_rfrtools.sh
...
clone_grovepi() {
  ...
  git clone --quiet --depth=1 -b $selectedbranch https://github.com/chaosAD/GrovePi.git
  cd $GROVEPI_DIR
}
